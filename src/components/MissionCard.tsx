...
export default function MissionCard({ name, id, status, priority, tca, coverImageId }: MissionCardProps) {
    ...
    return (
        <Link
            to={`/mission/${id}`}
            className='group relative block bg-gray-800 rounded-lg overflow-hidden shadow-lg transform transition-transform duration-300 hover:scale-105 border border-transparent hover:border-blue-500'
        >
            <img
                src={imageUrl}
                alt={`Cover for ${name}`}
